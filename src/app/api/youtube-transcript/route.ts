...
// Handle both YouTube's XML caption format and VTT (some Piped instances return VTT)
function parseCaptionBody(body: string): string {
  ...
  if (trimmed.startsWith('WEBVTT') || trimmed.includes('-->')) {
    ...
    for (const line of body.split(/\r?\n/)) {
      ...
      out.push(decodeEntities(t.replace(/<[^>]+>/g, '')));
    }
...
async function fetchViaPiped(videoId: string): Promise<VideoInfo | null> {
  const instances = [
  ...
  for (const base of instances) {
    try {
      ...
      clearTimeout(t);
      if (!res.ok) {
        console.error(`[yt-transcript] Piped ${base} returned ${res.status}`);
        continue;
      }
      const json = await res.json();
      const subs: any[] = json?.subtitles ?? [];
      if (!Array.isArray(subs) || subs.length === 0) {
        console.error(`[yt-transcript] Piped ${base}: no subtitles in response`);
        continue;
      }
      const tracks: CaptionTrack[] = subs.map((s) => ({
        baseUrl: s.url,
        languageCode: s.code,
        kind: s.autoGenerated ? 'asr' : undefined,
      }));
      const title: string = json?.title ?? 'YouTube video';
      console.log(`[yt-transcript] Piped ${base}: found ${tracks.length} subtitles`);
      return { tracks, title };
    } catch (e: any) {
      console.error(`[yt-transcript] Piped ${base} threw:`, e?.message);
    }
  }
  return null;
}
...
export async function POST(req: NextRequest) {
  ...
  if (!info) info = await fetchViaWatchPage(videoId);
  if (!info) info = await fetchViaPiped(videoId);
  if (!info) {
