...
export function ExtractFromNotes({ onClose }: Props) {
  ...
  useEffect(() => {
    fetch('/api/workspaces')
      ...
      .then((data: Workspace[]) => {
        ...
        const ids = new Set(data.flatMap((w: Workspace) => w.databases.map((d) => d.id)));
  ...
  async function handleExtract() {
    ...
    try {
      ...
      if (data.changes.length === 0) {
        setError('No relevant items found in these notes. Try selecting more databases or adding more context.');
        setLoading(false);
        return;
      }
  ...
  const allDatabases = workspaces.flatMap((w) => w.databases.map((d) => ({ ...d, workspaceName: w.name })));
          ...
          <div className="flex flex-col overflow-hidden">
            <div className="px-5 pt-4 pb-2 shrink-0">
              <p className="text-xs text-muted uppercase tracking-wide font-medium">
                Proposed changes &mdash; {changes.length} operation{changes.length !== 1 ? 's' : ''}
              </p>
            </div>
                ...
                <div key={i} className="rounded-lg border border-border bg-bg p-3 text-sm">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {Object.entries(c.action === 'update' ? c.changes : c.row)
                      .filter(([k]) => k !== 'Name' || c.action === 'create')
                      .map(([k, v]) => (
                        <span
                          key={k}
