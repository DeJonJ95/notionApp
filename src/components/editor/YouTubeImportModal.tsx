...
export function YouTubeImportModal({ onClose, onImport }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-bg border border-border rounded-2xl shadow-2xl w-full max-w-md flex flex-col">
        ...
        <div className="px-5 py-4 flex flex-col gap-3">
          <label className="text-sm text-muted">YouTube URL or video ID</label>
          <input
            value={url}
          ...
          <p className="text-xs text-muted/80 leading-relaxed">
            Pulls captions directly from YouTube — no Whisper cost. Pasted as a new
            text block; use the Summarize button on it afterwards if it&apos;s long.
          </p>
        ...
        <div className="flex items-center gap-2 px-5 py-3 border-t border-border">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 py-2 rounded-lg bg-accent text-white text-sm font-medium hover:bg-accent/80 disabled:opacity-40 transition-colors"
          >
